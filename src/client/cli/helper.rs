...
impl Session {
    pub fn new() -> Self {
        let home_folder = std::env::home_dir().map(|h|h.as_path());
        let config_folder = home_folder.clone()
            .map(|h| h / ".config" / "fabric");
        let sessions_folder = config_folder.map(|h| h / "sessions").map(Into::into).unwrap();
        match std::fs::try_exists(sessions_folder) {
            Ok(p) if !p => std::fs::create_dir_all(sessions_folder),
            _ => Ok(()),
        }.unwrap();
