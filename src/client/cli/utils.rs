...
impl AgentSetup {
    ...
    fn api_keys() -> Result<()> {
        ...
        println!("Welcome to Fabric. Let's get started.");
        let browserless = input("Please enter your Browserless API key\n");
        let serper = input("Please enter your Serper API key\n");

        let bl_entry = format!{"BROWSERLESS_API_KEY={}", browserless};
        let sp_entry = format!("SERPER_API_KEY={}", serper);
...
impl Config {
    fn new() -> Config {
        Config {
            browserless_api_key: "".to_string(),
            serper_api_key: "".to_string()
        }
    }

    fn set_browserless_api_key(&mut self, api_key: &str) {
        self.browserless_api_key = api_key.to_string();
    }

    fn set_serper_api_key(&mut self, api_key: &str) {
        self.serper_api_key = api_key.to_string();
    }
}

fn main() -> Result<(), String> {
    // Print welcome message and get API keys
    println!("Welcome to Fabric. Let's get started.");
    let browserless_api_key = read_line().and_then(|input: String| {
        prompt("Please enter your Browserless API key\n", input).ok()?.strip()
    });
    let serper_api_key = read_line().and_then(|input: String| {
        prompt("Please enter your Serper API key\n", input).ok()?.strip()
    });

    // Deserialize and store the data in memory
    let mut config = Config::new();
    config.set_browserless_api_key(browserless_api_key);
    config.set_serper_api_key(serper_api_key);

    // Check and write to file
    let env_file_path = "./env.json";
    if let Some(path) = PathBuf::new().join(&env_file_path) {
        if path.exists() {
            // Read and serialize the configuration data to a json string
            ...
            // Add the new entries to the beginning of the file
            filesystem::write(path, format!("{}\n", &json).as_bytes() as i32)?;
        } else {
            fs::File::create(path)?;
            config.to_string().serialize_to(&mut file)?;
        }
    }
...
fn prompt<R: BufRead>(message: &str, mut input: String) -> Result<String, String> {
    print!("{}", message);
    read_line(&mut input).ok()
}
