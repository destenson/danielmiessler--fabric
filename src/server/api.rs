...
const ALLOWLIST_PATTERN: &str = r"^[a-zA-Z0-9\s.,;:!?\-]+$";
...
fn sanitize_content(content: &str) -> String {
    ...
    let re = Regex::new(ALLOWLIST_PATTERN).unwrap();

    // Sanitize the content by removing characters that do not match the ALLOWLIST_PATTERN.
    content.chars() // Iterate through each character of the string
        .filter(|c| {
            let mut s = &"";
            s = format!("{} {}", c, *s);
            re.is_match(s)
        }) // Filter out characters that do not match the allowlist
        .collect::<String>() // Convert the filtered characters into a string
